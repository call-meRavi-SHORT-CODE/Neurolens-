...
from helper import NewEyeDataset, SiameseSeResNeXtdropout
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, scheduler, modelName, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (combined_image, labels, _) in progress_bar:
        ...
        # train_later_loss, train_later_acc = validate_model(model, train_loader, criterion, epoch,log_file=r'./Train_later_log.txt')
...
# valid function
def validate_model(model, valid_loader, criterion, epoch, batch_size=14, log_file=r'./valid_lognew.txt'):
    model.eval()
    running_loss = 0.0
    running_corrects = 0

    with torch.no_grad():
        for i, (combined_image, labels, _) in enumerate(valid_loader):
            combined_image = combined_image.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)
...
def main(json_file,PATH, weight_path, model, include_0_9mm=True, num_epochs=200, lr=0.0001, batch_size=14):
    json_file = json_file
    ...
    class_weights = torch.tensor([weight_1*1.1, weight_0], dtype=torch.float).cuda()
...
if __name__ == '__main__':
    # Global parameters
    PATH = r'replace-ur-training-data-path'  # training data path
    weight_path = r"cimt_model.pth"
    json_file=r"replace-ur-json-file-path"
    model = SiameseSeResNeXtdropout(dropout_p=0, spatial_dropout_p=0, out=2).to(device)
    modleName = model.__class__.__name__
    num_epochs = 200
    lr = 0.0001
    batch_size = 40
    include_0_9mm =False
    main(json_file=json_file,PATH=PATH, batch_size=batch_size, include_0_9mm=include_0_9mm, weight_path=weight_path, model=model,
