...
const Signup = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(formRef.current?.children,
        { opacity: 0, x: 30 },
        { opacity: 1, x: 0, duration: 0.8, stagger: 0.08, delay: 0.7, ease: 'power2.out' }
      );
    });
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left Side - Form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-[#0a0e1a] relative overflow-y-auto">
        <Card ref={cardRef} className="w-full max-w-md bg-transparent border-none shadow-none my-8" style={{ fontFamily: "'Poppins', system-ui, -apple-system, BlinkMacSystemFont, sans-serif" }}>
            ...
            <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  {...register('name')}
                  className={`h-14 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 focus:border-sky-400 focus:ring-2 focus:ring-sky-400/30 transition-all text-base ${errors.name ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30' : ''}`}
                />
                {errors.name && (
              ...
              <div className="space-y-2">
                <Label htmlFor="hospital_name" className="text-white font-semibold text-base">
                  Hospital Name
                </Label>
                <Input
                  id="hospital_name"
                  ...
                  className={`h-14 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 focus:border-sky-400 focus:ring-2 focus:ring-sky-400/30 transition-all text-base ${errors.hospital_name ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30' : ''}`}
                />
              ...
              <div className="space-y-2">
                <Label htmlFor="hospital_location" className="text-white font-semibold text-base">
                  Hospital Location
                </Label>
                <Input
                  ...
                  className={`h-14 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 focus:border-sky-400 focus:ring-2 focus:ring-sky-400/30 transition-all text-base ${errors.hospital_location ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30' : ''}`}
                />
              ...
              <div className="space-y-2">
                <Label htmlFor="confirmPassword" className="text-white font-semibold text-base">
                  Confirm Password
                </Label>
                <div className="relative">
                  <Input
                    ...
                    className={`h-14 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-500 focus:border-sky-400 focus:ring-2 focus:ring-sky-400/30 transition-all pr-12 text-base ${errors.confirmPassword ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30' : ''}`}
                  />
                  <Button
                    type="button"
                    variant="ghost"
              ...
              <Button
                type="submit"
                className="w-full h-14 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-semibold text-base shadow-lg shadow-sky-500/20 transition-all transform hover:scale-[1.02] active:scale-[0.98] mt-2"
                disabled={loading}
              >
                {loading ? (
      ...
      {/* Right Side - Gradient Panel with Medical Images */}
      <div ref={logoRef} className="hidden lg:flex lg:w-1/2 bg-[#0a0e1a] relative overflow-hidden items-end justify-start p-12">
        {/* Doctor Image */}
        <div className="absolute inset-0">
          <img src="/src/assets/doctor.jpeg" alt="Medical Professional" className="w-full h-full object-cover opacity-70" />
          <div className="absolute inset-0 bg-gradient-to-br from-[#0a0e1a]/50 via-[#0f1419]/30 to-[#1a2332]/20"></div>
        </div>
